...
module.exports = class Protocol12ReaderClient extends Writable {
  ...
  [_serverHello]() {
    ...
    try {
      const istream = createDecodeStream(handshake.body);
  ...
  [_serverKeyExchange]() {
    ...
    // Only ECDHE_* may use this message.
    if (this.session.nextCipher.kx.signType !== signTypes.ECDHE) {
      throw new Error('Invalid key exchange type.');
    }

    const rstream = createDecodeStream(handshake.body);
    ...
    if (this.session.nextCipher.kx.keyType === keyTypes.RSA) {
      assert(
        digitalSign.algorithm === signatureScheme.rsa_pkcs1_sha1 ||
        digitalSign.algorithm === signatureScheme.rsa_pkcs1_sha256 ||
        digitalSign.algorithm === signatureScheme.rsa_pkcs1_sha384 ||
        digitalSign.algorithm === signatureScheme.rsa_pkcs1_sha512 ||
        digitalSign.algorithm === signatureScheme.rsa_pss_rsae_sha256 ||
        digitalSign.algorithm === signatureScheme.rsa_pss_rsae_sha384 ||
        digitalSign.algorithm === signatureScheme.rsa_pss_rsae_sha512
      );
    }
    ...
    if (this.session.nextCipher.kx.keyType === keyTypes.ECDSA) {
      assert(
        ...
        digitalSign.algorithm === signatureScheme.ecdsa_secp384r1_sha384 ||
        digitalSign.algorithm === signatureScheme.ecdsa_secp521r1_sha512 ||
        digitalSign.algorithm === signatureScheme.ed25519 ||
        digitalSign.algorithm === signatureScheme.ed448
       );   
    }
  ...
  [_certificateRequest]() {
    ...
    const rstream = createDecodeStream(handshake.body);
    const certificateRequest = decode(rstream, CertificateRequest);
